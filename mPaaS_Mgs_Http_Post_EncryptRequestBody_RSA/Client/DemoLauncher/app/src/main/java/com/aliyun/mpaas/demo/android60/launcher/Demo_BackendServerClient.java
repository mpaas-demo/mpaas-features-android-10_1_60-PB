// Generated by HybirdPB CodeGenerator(1.2.0). DO NOT EDIT!
// Generation date is 2020-03-02 05:45:02

package com.aliyun.mpaas.demo.android60.launcher;

import com.alipay.mobile.framework.service.ext.annotation.CheckLogin;
import com.alipay.mobile.framework.service.annotation.OperationType;
import com.alipay.mobile.framework.service.annotation.SignCheck;

import com.aliyun.mpaas.demo.android60.launcher.request.PathMpaasMgsHttpPostEncryptrequestbodyRsaPostReq;
   
public interface Demo_BackendServerClient {

    @SignCheck
    @OperationType("op_mPaaS_Mgs_Http_Post_EncryptRequestBody_RSA") 
    public String pathMpaasMgsHttpPostEncryptrequestbodyRsaPost(PathMpaasMgsHttpPostEncryptrequestbodyRsaPostReq requestParam);


}