# mPaaS_Mgs_Http_Get_QueryStr

## 描述
本Demo演示APP如何通过mPaaS移动网关(MGS)访问HTTP方法为 GET 的http api接口，并通过Query String传参。

本demo中后端http被调用方式为：

    GET http://114.55.252.107/path_mPaaS_Mgs_Http_Get_QueryStr?querystring1=123
...
##### 2.在公网可访问的服务器中，运行该jar包启动后端服务。 
  ...
  例如本例在一台IP为114.55.252.107的服务器上运行该jar包，则该后端服务通过URL http://114.55.252.107/path_mPaaS_Mgs_Http_Get_QueryStr对外提供HTTP方法为get的http api服务。
  
  *服务部署后，可以通过浏览器访问http://114.55.252.107/path_mPaaS_Mgs_Http_Get_QueryStr?querystring1=123 ，以验证服务是否正常运行*
...
- operationType: 对operationType命名。 operationType可以理解为一个具体的api接口。例如实际生产中，对于上述提及的后端登录服务系统的一个登录服务，这里可以填写为login或LoginAPI等用于辨识该接口的命名。因本demo不含任何业务属性，我们对后端demo的接口api在这里命名为op_mPaaS_Mgs_Http_Get_QueryStr。

##### 3.点击 mPaaS控制台 ->后台服务管理->移动网关->API管理->op_mPaaS_Mgs_Http_Get_QueryStr->基础信息-修改 ， 
...
- API名称：在实际项目中填写该api接口(operationType)对操作者友好的有意义名称作为api注释，例如“登录”，“查询余额”。 本Demo因不含任何业务属性，填写了ApiName_mPaaS_Mgs_Http_Get_QueryStr
- 接入系统：选择步骤1中创建的API分组 Demo_BackendServer
- 请求path: 在实际项目中，请求path为后端业务系统实际http api接口的url的路径部分，例如/login , /checkBalance 。 本demo中，后端demo服务提供的api路径为 /path_mPaaS_Mgs_Http_Get_QueryStr 
- 调用方式：后端api的HTTP方法类型。本demo中为GET.
...
##### 5.点击 mPaaS控制台 ->后台服务管理->移动网关->API管理->op_mPaaS_Mgs_Http_Get_QueryStr的更多->生成代码
